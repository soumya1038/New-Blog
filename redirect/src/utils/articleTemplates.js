...
const escapeTemplateJson = (value) => escapeHtml(JSON.stringify(value));

const safeTemplatePath = (value) => {
  const candidate = cleanText(value, '');
  if (!candidate || !candidate.startsWith('/') || candidate.startsWith('//') || /[\\]/.test(candidate)) return '';

  try {
    const parsed = new URL(candidate, 'https://lekhon.local');
    if (parsed.origin !== 'https://lekhon.local') return '';
...
const safeTemplateHref = (value, fallback = '#') =>
  safeTemplatePath(value) || getSafeHttpUrl(value) || fallback;

const safeTemplateImageSrc = (value, fallback = '') =>
  safeTemplatePath(value) || getSafeHttpUrl(value) || fallback;
...
export const normalizeArticleTemplateData = (article = {}) => {
  ...
  const coverImage = safeTemplateImageSrc(article.coverImage);
  const galleryImages = parseGalleryImages(article.galleryImages)
    .map((url) => safeTemplateImageSrc(url))
    .filter((url, index, list) => url && url !== coverImage && list.indexOf(url) === index);
...
const inlineMarkdown = (line) =>
  escapeHtml(line)
    ...
    .replace(INLINE.link, (match, label, href) => {
      const safeHref = safeTemplateHref(href, '');
      if (!safeHref) return label;
      const external = /^https?:\/\//i.test(safeHref);
      return `<a href="${escapeHtml(safeHref)}"${external ? ' target="_blank" rel="noopener noreferrer" referrerpolicy="strict-origin-when-cross-origin"' : ''}>${label}</a>`;
    });
...
const renderInlineMedia = (url, index) => {
  const src = safeTemplateImageSrc(url);
  if (!src) return '';
  return '<figure class="inline-media reveal"><img src="' +
    escapeHtml(src) +
    '" alt="Story visual ' +
    (index + 1) +
    '" loading="lazy" referrerpolicy="no-referrer" /><figcaption>Story visual ' +
    (index + 1) +
    '</figcaption></figure>';
};
...
const renderContent = (content, showDropCap = true, inlineMedia = [], options = {}) => {
  ...
  const safeMedia = Array.isArray(inlineMedia) ? inlineMedia.map((url) => safeTemplateImageSrc(url)).filter(Boolean) : [];
...
const renderVideos = (videoUrls) => {
  ...
  const cards = videoUrls
    .map((url) => {
      const renderInfo = getSafeVideoRenderInfo(url);
      if (!renderInfo) return '';

      if (renderInfo.type === 'embed') {
        return `<div class="video-card reveal"><iframe src="${escapeHtml(renderInfo.src)}" title="Embedded video" loading="lazy" allow="${escapeHtml(SAFE_EMBED_IFRAME_ALLOW)}" sandbox="${escapeHtml(SAFE_EMBED_IFRAME_SANDBOX)}" referrerpolicy="${escapeHtml(SAFE_EMBED_REFERRER_POLICY)}" allowfullscreen></iframe></div>`;
      }

      return `<div class="video-card reveal"><video controls preload="metadata"><source src="${escapeHtml(renderInfo.src)}" /></video></div>`;
    })
    .filter(Boolean)
    .join('');
...
const renderCover = (article, className = '') => {
  if (article.coverImage) {
    return `<figure class="cover frame-card reveal ${className}"><img src="${escapeHtml(article.coverImage)}" alt="Article cover" loading="lazy" referrerpolicy="no-referrer" /><figcaption>${escapeHtml(article.category)} | Visual Brief</figcaption></figure>`;
  }
...
const renderCustomStudioImage = (article, block) => {
  const imageSource = safeTemplateImageSrc(article.coverImage, safeTemplateImageSrc(article.galleryImages[0]));
...
const collectGallerySources = (article) =>
  [article.coverImage, ...(article.galleryImages || [])]
    .map((item) => safeTemplateImageSrc(item))
...
const renderCustomStudioVideo = (article, block) => {
  ...
  const embeds = article.videoUrls
    ...
    .map((url) => {
      ...
      if (renderInfo.type === 'embed') {
        return `<div class="custom-studio-video-card"><iframe src="${escapeHtml(renderInfo.src)}" title="Embedded video" loading="lazy" allow="${escapeHtml(SAFE_EMBED_IFRAME_ALLOW)}" sandbox="${escapeHtml(SAFE_EMBED_IFRAME_SANDBOX)}" referrerpolicy="${escapeHtml(SAFE_EMBED_REFERRER_POLICY)}" allowfullscreen></iframe></div>`;
      }

      return `<div class="custom-studio-video-card"><video controls preload="metadata"><source src="${escapeHtml(renderInfo.src)}" /></video></div>`;
    })
    .filter(Boolean)
    .join('');

  if (!embeds) {
    return '<div class="custom-studio-video-fallback"><h3>Video Brief</h3><p>Add one or more valid video URLs to fill this section.</p></div>';
  }
...
const renderCustomStudioProductTagAnchor = (article, showPlaceholder = false) => {
  ...
  const safeFirstImage =
    safeTemplateImageSrc(firstProduct?.transparentThumbnail)
    || safeTemplateImageSrc(firstProduct?.thumbnail)
    || safeTemplateImageSrc(firstExternal?.thumbnail)
    || '';
  const summaryLabel = `${total} product${total === 1 ? '' : 's'}`;
  const productRows = products
    .map((product) => {
      const image = safeTemplateImageSrc(product.transparentThumbnail, safeTemplateImageSrc(product.thumbnail, '/image/lekhon_url.png'));
      const price = productPriceLabel(product);
      return `
        <a class="custom-product-tag-row" href="${escapeHtml(safeTemplateHref(productHref(product), '/marketplace'))}">
          <img src="${escapeHtml(image)}" alt="${escapeHtml(product.title || 'Tagged product')}" loading="lazy" referrerpolicy="no-referrer" />
  ...
  const externalRows = externalLinks
    .map((link) => {
      const thumbnail = safeTemplateImageSrc(link.thumbnail);
      return `
        <a class="custom-product-tag-row" href="${escapeHtml(safeExternalHref(link.url))}" target="_blank" rel="noopener noreferrer" referrerpolicy="strict-origin-when-cross-origin">
          ${
            thumbnail
              ? `<img src="${escapeHtml(thumbnail)}" alt="${escapeHtml(link.title || 'External product')}" loading="lazy" referrerpolicy="no-referrer" />`
    ...
    <div class="custom-product-tag-anchor" aria-label="Tagged products">
      <details class="custom-product-tag-overlay">
        <summary class="custom-product-tag-summary">
          ${
            safeFirstImage
              ? `<img class="custom-product-tag-image" src="${escapeHtml(safeFirstImage)}" alt="${escapeHtml(firstProduct?.title || firstExternal?.title || 'Tagged product')}" loading="lazy" referrerpolicy="no-referrer" />`
              : ''
          }
...
const renderTemplateHtml = (article, template) => {
  ...
  const runtimeData = {
    heightPadding: template.id === CUSTOM_TEMPLATE_ID ? 0 : 56,
    paginationConfig,
    storyText: (article.paragraphs || []).join(' ').trim() || article.metaDescription || '',
    summaryText: buildReaderSummary(article).join(' ')
  };
...
<body class="theme-${resolvedThemeMode} layout-${template.resolvedLayout} style-${template.style} template-${template.id} ${longReadClass}">
  ...
  <template id="article-template-runtime-data">${escapeTemplateJson(runtimeData)}</template>
  <script defer src="/article-template-runtime.js"></script>
</body>
</html>`;
};
