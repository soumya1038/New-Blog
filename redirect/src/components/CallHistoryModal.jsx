...
const CallHistoryModal = ({ callLogs, onClose, onCallBack, getUserDisplayName, getUserAvatar, currentUserId, onDeleteLog }) => {
  ...
  return (
    <div className="fixed inset-0 theme-modal-overlay z-[60] flex items-center justify-center p-4">
      <div className="theme-modal-card rounded-lg shadow-xl max-w-md w-full max-h-[80vh] flex flex-col">
        {/* Header */}
        <div className="p-4 border-b border-[var(--border-default)] flex items-center justify-between">
          <h3 className="text-lg font-semibold text-[var(--text-primary)]">Call History</h3>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {callLogs.length === 0 ? (
            <div className="text-center py-8 text-[var(--text-muted)]">
              <FiPhone className="w-12 h-12 mx-auto mb-2 text-[var(--text-muted)]" />
            ...
            <div className="space-y-3">
              {callLogs.map((log) => {
                ...
                <div
                  key={log._id}
                  className="flex items-center gap-3 p-3 hover:bg-[var(--surface-elevated)] rounded-lg transition-colors"
                >
                  {/* User Avatar */}
                  <img
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {log.type === 'video' ? (
                        <FiVideo className="w-4 h-4 text-[var(--text-muted)]" />
                      ) : (
                        <FiPhone className="w-4 h-4 text-[var(--text-muted)]" />
                      )}
                    </div>
