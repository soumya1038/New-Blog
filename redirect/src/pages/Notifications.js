...
const Notifications = () => {
  ...
  return (
    <>
      {showModal && (
        <div className="fixed inset-0 theme-modal-overlay flex items-center justify-center z-50 px-4">
          <div className="theme-modal-card rounded-xl shadow-2xl max-w-sm w-full p-6 animate-fadeIn">
            <h3 className="text-lg font-semibold text-[var(--text-primary)] mb-2">{t('Clear All Notifications?')}</h3>
            <p className="text-[var(--text-secondary)] mb-6">{t('This action cannot be undone. All notifications will be permanently deleted.')}</p>
            <div className="flex gap-3">
              <button
                onClick={() => setShowModal(false)}
                className="flex-1 px-4 py-2 theme-soft-button rounded-lg transition-colors font-medium"
              >
                {t('Cancel')}
              </button>
              <button
      ...
      <div className="min-h-screen bg-gradient-to-br from-[var(--background-secondary)] via-[var(--background-primary)] to-[var(--background-secondary)] py-8">
      <div className="container mx-auto px-4 max-w-3xl">
        <button
          onClick={() => navigate(-1)}
          className="flex items-center gap-2 mb-4 font-semibold text-[var(--brand-primary)] hover:opacity-80"
        >
          <FaArrowLeft /> {t('Back')}
        </button>
        <div className="theme-modal-card rounded-2xl shadow-xl p-4 sm:p-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3">
            <h1 className="text-2xl sm:text-3xl font-bold text-[var(--text-primary)]">{t('Notifications')}</h1>
