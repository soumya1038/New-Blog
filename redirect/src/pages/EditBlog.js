...
const EditBlog = () => {
  ...
  const fetchBlog = async () => {
    try {
      ...
      setRemovedGalleryPublicIds([]);
      if (mode === 'article') {
        setSelectedArticleTemplateId(data.blog.templateId || DEFAULT_ARTICLE_TEMPLATE_ID);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center relative" style={{ background: isDark ? 'var(--background-primary)' : 'linear-gradient(180deg, var(--background-primary) 0%, var(--background-secondary) 100%)' }}>
